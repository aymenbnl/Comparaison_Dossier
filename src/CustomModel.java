...
public class CustomModel {
	...
	public static DefaultTableModel getModel(LinkedList<File > listFiles,File file2,DefaultTableModel model){  
		for (File f : listFiles) {
			if (f.isDirectory()){
				if( ( Belong(f,file2)==false)){    //cette dossier n'existe pas dans le 2eme repertoire
					...
					LinkedList<File> fs=new LinkedList<File>();      //mettre le contenu de r�pertoire dans une list
					if (f.listFiles()!=null) {
	...
	//v�rifier si le repertoire est existe ou pas
	...
	//v�rifier si la date de folder est chang� ou non et si le fichier est existe ou pas
	...
	//v�rifier si la taille de folder est chang� ou non et si le fichier est existe ou pas
